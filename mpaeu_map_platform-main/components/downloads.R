...
observe({
  ...
  fs::dir_create(fs::path(tfold, directories))
  for (k in seq_along(sel_files)) {
    download.file(paste0(addr, sel_files[k]), fs::path(tfold, gsub("results/species/", "", sel_files[k])))
  }
  #download.file(paste0(addr, sel_files), fs::path(tfold, sel_files), method = "libcurl")
