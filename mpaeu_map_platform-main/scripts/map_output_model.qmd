...
library(ggplot2)
library(dplyr)
source(fs::path("{WD_PATH}", "/scripts/auxiliary_modelfit.R"))
outfolder <- fs::path("{WD_PATH}", "/data/maps")#"../data/maps/"
model_acro <- "{ACRO_CODE}"
s3_list <- arrow::open_dataset(fs::path("{WD_PATH}", "/data/s3_list.parquet"))
...
masks <- terra::rast(retrieve_s3(sp, "mask_cog.tif$", s3_list))

ecoreg <- sf::st_read(fs::path("{WD_PATH}", "/data/MarineRealms_BO.shp"))

logf <- jsonlite::read_json(retrieve_s3(sp, "log.json$", s3_list))
...
```{r echo=FALSE}
pts <- arrow::read_parquet(retrieve_s3(sp, "fitocc", s3_list))
...
cv <- arrow::read_parquet(retrieve_s3(sp, paste0("method=",which_model,"_what=cvmetrics"), s3_list))
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
imp <- arrow::read_parquet(retrieve_s3(sp, paste0("method=",which_model,"_what=varimp"), s3_list))
knitr::kable(imp)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
rc <- arrow::read_parquet(retrieve_s3(sp, paste0("method=",which_model,"_what=respcur"), s3_list))
...
Note: for this report, raster layers are aggregated at 0.5 degree resolution.
